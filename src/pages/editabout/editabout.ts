...
export class EditaboutPage {
  ...
  getprovinces() {
      let url = "http://10.0.2.192/servicewa/getprovinces.php";
  ...
  getamphures() {
    let url = 'http://10.0.2.192/servicewa/getdistricts.php?province_id='+this.id;
    this.http.get(url).subscribe((data: any) => {
  ...
  showte(a){
   ...
   gettambon() {
    let url = 'http://10.0.2.192/servicewa/gettambon.php?amphure_id='+this.ida;
    this.http.get(url).subscribe((data: any) => {
  ...
  save(name,surname,phone,email,subdistricts){
    ...
    let url = 'http://10.0.2.192/servicewa/updateabout.php?id='+val; //ให้ไป post ที่  url
